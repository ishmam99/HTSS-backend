...
class TrainingCourseScheduleController extends Controller
{
    ...
    {
        // Get all months that have schedules (use selectRaw to avoid DB facade dependency)
        $monthsWithSchedules = TrainingCourseSchedule::selectRaw('YEAR(`date`) as year, MONTH(`date`) as month, COUNT(*) as total_schedules')
            ->groupBy('year', 'month')
            ->orderBy('year', 'desc')
            ->orderBy('month', 'desc')
            ->get();
